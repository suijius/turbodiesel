...
import localization
import data
from metamodel.models import Entity, Property, Application, create_model, get_application_instance, get_entity_instance, get_model
import settings
            ...
            if type(params_dict[key]) is ForeignRelatedObjectsDescriptor:
                ...
                if len(entity_list):
                    if not entity_list[0].service:
                        ...
                        params_ext[related_object.model.entity] = get_params(request, application_alias, related_object.model.entity.alias)
        return self.render_to_response(
            context=self.get_context_data(form=form, request=request, application=application,
