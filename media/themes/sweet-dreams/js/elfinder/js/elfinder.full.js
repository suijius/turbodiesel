        ...
        this.reload = function (data) {
            ...
            if (this.options.autoReload > 0) {
                ...
                this.iID = setInterval(function () {
                    !self.locked && self.ui.exec('reload');
...
(function ($) {
    elFinder.prototype.view = function (fm, el) {
        ...
        this.err = $('<p class="el-finder-err"><strong/></p>').click(function () {
            $(this).hide();
        });
    ...
    elFinder.prototype.ui = function (fm) {
        ...
        this.showMenu = function (e) {
            ...
            this.menu.css({
                ...
                function () {
                    $(this).removeClass('hover').children('div').hide();
                }
            ).click(function (e) {
                    e.stopPropagation();
                    var t = $(this);
        ...
        this.init = function (disabled) {
            ...
            if (this.fm.options.docked) {
                this.dockButton.hover(
                    function () {
                        $(this).addClass('el-finder-dock-button-hover')
                    },
                    function () {
                        $(this).removeClass('el-finder-dock-button-hover')
                    }
                ).click(function () {
...
(function ($) {
    elFinder.prototype.quickLook = function (fm, el) {
        ...
        this.win = $('<div class="el-finder-ql"/>').hide()
            .append($('<div class="el-finder-ql-drag-handle"/>').append($('<span class="ui-icon ui-icon-circle-close"/>').click(function () {
                self.hide();
            })).append(this.title))
            .append(this.ico)
            .append(this.media)
            ...
            this.show = function (ql, f) {
                ...
                if (ql.mimes[f.mime] && f.hash == ql._hash) {
                    $('<img/>').hide().appendTo(ql.media.show()).attr('src', f.url + ($.browser.msie || $.browser.opera ? '?' + Math.random() : '')).load(function () {
                        t = $(this).unbind('load');
                        if (f.hash == ql._hash) {
                            ql.win.is(':animated') ? setTimeout(function () {
                                preview(t);
                            }, 330) : preview(t);
                        }
                    });
            ...
            if ($.browser.msie) {
                $('*', this.cwd).attr('unselectable', 'on')
                    ...
                    .bind('dragstart', function () {
                        self.cwd.selectable('disable').removeClass('ui-state-disabled ui-selectable-disabled');
                    })
