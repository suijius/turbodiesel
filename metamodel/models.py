...
class Property(models.Model):
    ...
    link_entity = models.ForeignKey(Entity, verbose_name=u'Ссылка на сущность', blank=True, null=True, related_name='link_entity')
...
class FilterOnPage(models.Model):
    ...
    page = models.ForeignKey(Page, db_column='page_id', verbose_name=u'Страница приложения', blank=False, null=False, related_name='page')
    filter = models.ForeignKey(ExtFilter, db_column='filter_id', verbose_name=u'Фильтр', blank=False, null=False, related_name='filter')
    class Meta:
...
class ExtStatus(models.Model):
    ...
    name = models.CharField(max_length=255, verbose_name=u'Имя')
    workflow = models.ForeignKey(ExtWorkflow, db_column='workflow_id', verbose_name=u'Рабочий процесс', blank=False, null=False, related_name='workflow')
    class Meta:
...
class ExtEdge(models.Model):
    ...
    target = models.ForeignKey(ExtStatus, db_column='target_id', verbose_name=u'Следующий статус', blank=False, null=False, related_name='target')
    class Meta:
        db_table = u'ext_edge'
    ...
    if entity.alias != "":
        ...
        for property in Property.objects.filter(parent_entity = entity):
            ...
            if property_type.dbtype == 'DATE':
                properties[property.name] = models.DateField(db_column=property.name, verbose_name=property.label, blank=True, null=True)
            elif property_type.dbtype == 'DATETIME':
                properties[property.name] = models.DateTimeField(db_column=property.name, verbose_name=property.label, blank=True, null=True)
            elif property_type.dbtype == 'INTEGER':
                properties[property.name] = models.IntegerField(db_column=property.name, verbose_name=property.label, blank=True, null=True)
            elif property_type.dbtype == 'DOUBLE':
                properties[property.name] = models.FloatField(db_column=property.name, verbose_name=property.label, blank=True, null=True)
            elif property_type.dbtype == 'BOOLEAN':
                properties[property.name] = models.BooleanField(db_column=property.name, verbose_name=property.label, default=True)
            elif property_type.dbtype == 'TEXT':
                properties[property.name] = models.TextField(db_column=property.name, verbose_name=property.label, blank=True, null=True)
            elif property_type.dbtype == 'STRING':
                properties[property.name] = models.CharField(max_length=1000, db_column=property.name, verbose_name=property.label, blank=True, null=True)
            elif property_type.dbtype == 'IMAGE':
                properties[property.name] = models.ImageField(upload_to='turbodiesel/images/entity/' + entity.alias, db_column=property.name, verbose_name=property.label, blank=True, null=True)
                ...
                properties[property.name] = models.ForeignKey(globals()['nature_' + property.link_entity.alias], db_column=property.name, verbose_name=property.label, blank=True, null=True, related_name='%s_%s' % (entity.alias, property.name))
            elif property.property_type.dbtype == 'EXTUSER':
                properties[property.name] = models.ForeignKey(User, db_column=property.name, verbose_name=property.label, related_name='%s_%s' % (entity.alias, property.name))

    properties['Meta'] = type('Meta', (), {'db_table':'nature_' + entity.alias, 'verbose_name_plural':entity.name.encode('utf8'), 'verbose_name':entity.name.lower().encode('utf8')})
...
def collect_entity():
    ...
    collection = [globals()[l] for l in globals().keys() if (type(globals()[l]) == django.db.models.base.ModelBase or type(globals()[l]) == mptt.models.MPTTModelBase)    
    ...
    for cl in collection:
        ...
        properties = properties + [property.__dict__['name'] for property in cl.__dict__['_meta'].__dict__['local_many_to_many']]
...
#        cladmin = type(st + 'Admin', (admin.ModelAdmin,), dict(fields=properties, list_per_page=100, list_filter=list_filter, search_properties=search_properties))
        cladmin = type(st + 'Admin', (admin.ModelAdmin,), dict(fields=properties, list_display=list_display, list_per_page=100, list_filter=list_filter, search_properties=search_properties))
        ...
        if inline.__contains__(cl):
            for entity in inline[cl]:
                entity_inline = type(str(entity.alias) + 'Inline', (admin.TabularInline,), dict(extra=1, model=globals()['nature_' + entity.alias]))
...
def pre_save_model(sender, **kwargs):
    if kwargs['instance']._meta.object_name == 'ExtImage':
        kwargs['instance'].image.field.upload_to =  'turbodiesel/images/application/%s/' % kwargs['instance'].application.alias
...
def post_save_model(sender, **kwargs):
    ...
    if kwargs['created']:
        if kwargs['instance']._meta.object_name == 'Entity':
            qset = Table.objects.using('metadata').values_list('table_name', 'table_schema').filter(table_name = kwargs['instance'].alias, table_schema = 'turbodiesel')
            if len(qset) == 0:
                sql = """
...
ALTER TABLE `nature_%s` ADD COLUMN `%s` %s NULL;
           """ % (kwargs['instance'].parent_entity.alias, kwargs['instance'].name, kwargs['instance'].property_type.dbtype)
            try:
                cursor.execute(sql)
...
def post_delete_model(sender, **kwargs):
    if kwargs['instance']._meta.object_name == 'Entity':
        ...
        sql = "ALTER TABLE `nature_%s` RENAME TO  `_delete_%s_nature_%s`;" % (kwargs['instance'].alias, delete_date, kwargs['instance'].alias)
        cursor.execute(sql)
        try:
...
def get_application_instance(application_alias, request):
    ...
    if request.META.get('PATH_INFO', 'hz') != '/':
        ...
        if len(instance_list):
            ...
            a = Attack.objects.create(ip=request.META.get('HTTP_X_REAL_IP', '127.0.0.1'), url=request.META.get('PATH_INFO', 'hz'), date_attack=datetime.datetime.now())
            transaction.commit()
