...
from turbodiesel.models import Property, ExtFilter, get_application_instance, get_model
...
def create_form(application_alias, model):
    properties = {'Meta': type('Meta', (), {'model': model})}
    return type(model._meta.objects_name, (ModelForm,), properties)
...
#     model_bag = get_model(request, 'bag', application_alias)
...
#         order = model_order(status=model_order_status.objects.get(request=1, application_alias=None),
...
#     if article:
#         post["article"] = model.objects.get(request=article, application_alias=None).id
#
#     form = create_form(application_alias, model_bag)(post, request.FILES)
#     if form.is_valid():
#         form.save()
...
#     model_ext = get_model(request, extension_alias, application_alias)
#     model_order = get_model(request, 'order', application_alias)
#     model_order_status = get_model(request, 'order_status', application_alias)
#     order_list = model_order.objects.filter(status=model_order_status.objects.get(request=1, application_alias=None),
#                                             session=request.session.session_key)
#     order = []
#     if len(order_list):
#         order = order_list[0]
...
#
#     post['status'] = 2
#     form = create_form(application_alias, model_ext)(post, request.FILES, instance=order)
#     if form.is_valid():
#         form.save()
#         return HttpResponseRedirect(request.META['HTTP_REFERER'])
#     else:
