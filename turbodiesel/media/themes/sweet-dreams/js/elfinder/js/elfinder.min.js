(function (a) {
    elFinder = function (d, g) {
        ...
        this.ajax = function (k, l, i) {
            var j = {url: this.options.url, async: true, type: "GET", data: k, dataType: "json", cache: false, lock: true, force: false, silent: false};
            if (typeof(i) == "object") {
                j = a.extend({}, j, i)
            }
            if (!j.silent) {
        ...
        this.tmb = function () {
            this.ajax({cmd: "tmb", current: b.cwd.hash}, function (k) {
        ...
        this.getPlaces = function () {
            var i = [], j = this.cookie(this.pCookie);
            if (j.length) {
                if (j.indexOf(":") != -1) {
                    i = j.split(":")
                } else {
        ...
        this.checkSelectedPos = function (k) {
            var j = b.view.cwd.find(".ui-selected:" + (k ? "last" : "first")).eq(0), l = j.position(), i = j.outerHeight(), m = b.view.cwd.height();
            if (l.top < 0) {
        ...
        this.setBuffer = function (k, m, o) {
            var j, n, l;
            this.buffer = {src: this.cwd.hash, dst: o, files: [], names: [], cut: m || 0};
        ...
        if (this.options.dialog || this.options.docked) {
            this.options.dialog = a.extend({width: 570, dialogClass: "", minWidth: 480, minHeight: 330}, this.options.dialog || {});
        ...
        this.ajax({cmd: "open", target: this.lastDir() || "", init: true, tree: true}, function (i) {
            if (i.cwd) {
    ...
    elFinder.prototype.options = {url: "", lang: "en", cssClass: "", wrap: 14, places: "Places", placesFirst: true, editorCallback: null, cutURL: "", closeOnEditorCallback: true, i18n: {}, view: "icons", width: "", height: "", disableShortcuts: false, rememberLastDir: true, cookie: {expires: 30, domain: "", path: "/", secure: false}, toolbar: [
        ["back", "reload"],
        ["select", "open"],
        ["mkdir", "mkfile", "upload"],
        ["copy", "paste", "rm"],
        ...
        ["icons", "list"],
        ["help"]
    ], contextmenu: {cwd: ["reload", "delim", "mkdir", "mkfile", "upload", "delim", "paste", "delim", "info"], file: ["select", "open", "quicklook", "delim", "copy", "cut", "rm", "delim", "duplicate", "rename", "edit", "resize", "archive", "extract", "delim", "info"], group: ["select", "copy", "cut", "rm", "delim", "archive", "extract", "delim", "info"]}, dialog: null, docked: false, autoReload: 0, selectMultiple: false};
...
(function (a) {
    elFinder.prototype.view = function (d, c) {
        ...
        this.renderNav = function (g) {
            var i = g.dirs.length ? h(g.dirs) : "", e = '<li><a href="#" class="el-finder-tree-root" key="' + g.hash + '"><div' + (i ? ' class="collapsed expanded"' : "") + "/>" + g.name + "</a>" + i + "</li>";
            this.tree.html(e);
        ...
        this.renderPlaces = function () {
            var g, j, h = this.fm.getPlaces(), e = this.plc.show().find("ul").empty().hide();
            a("div:first", this.plc).removeClass("collapsed expanded");
            if (h.length) {
                h.sort(function (k, i) {
                    var m = b.tree.find('a[key="' + k + '"]').text() || "", l = b.tree.find('a[key="' + i + '"]').text() || "";
...
(function (a) {
    elFinder.prototype.ui = function (c) {
        ...
        this.showMenu = function (i) {
            ...
            j(g);
            h = a(window);
            d = {height: h.height(), width: h.width(), sT: h.scrollTop(), cW: this.menu.width(), cH: this.menu.height()};
            this.menu.css({left: ((i.clientX + d.cW) > d.width ? (i.clientX - d.cW) : i.clientX), top: ((i.clientY + d.cH) > d.height && i.clientY > d.cH ? (i.clientY + d.sT - d.cH) : i.clientY + d.sT)}).show().find("div[name]").hover(function () {
                var l = a(this), m = l.children("div"), e;
                l.addClass("hover");
                if (m.length) {
                    if (!m.attr("pos")) {
                        e = l.outerWidth();
        ...
        this.init = function (k) {
            var h, d, o, m = false, g = 2, l, e = this.fm.options.toolbar;
    ...
    elFinder.prototype.ui.prototype.commands = {back: function (c) {
        var b = this;
        this.name = "Back";
        this.fm = c;
        this.exec = function () {
            if (this.fm.history.length) {
                this.fm.ajax({cmd: "open", target: this.fm.history.pop()}, function (d) {
                    b.fm.reload(d)
                })
            }
        };
        ...
        this.fm = c;
        this.exec = function () {
            this.fm.ajax({cmd: "open", target: this.fm.cwd.hash, tree: true}, function (d) {
                b.fm.reload(d)
            })
        };
        this.cm = function (d) {
        ...
        this.exec = function (e) {
            var g = null;
            if (e) {
                g = {hash: a(e).attr("key"), mime: "directory", read: !a(e).hasClass("noaccess") && !a(e).hasClass("dropbox")}
            } else {
            ...
            function h(i) {
                ...
                b.fm.ajax({cmd: "open", target: i}, function (j) {
                    b.fm.reload(j)
                })
            }
        ...
        this.fm = c;
        this.exec = function () {
            var j, i, e = this.fm.selected.length, d = a(window).width(), g = a(window).height();
            ...
            function k(m) {
                ...
                a("<div />").append(l + "</table>").dialog({dialogClass: "el-finder-dialog el-finder-dialog-info", width: 390, position: n, title: b.fm.i18n(m.mime == "directory" ? "Folder info" : "File info"), close: function () {
                    if (--e <= 0) {
                        b.fm.lockShortcuts()
                    }
        ...
        this.fm = c;
        this.exec = function () {
            var i = this.fm.getselected(), h, l, e, j, k;
            if (i.length == 1) {
                j = i[0];
            ...
            function d() {
                if (!b.fm.locked) {
                    ...
                    b.fm.ajax({cmd: "rename", current: b.fm.cwd.hash, target: j.hash, name: m}, function (o) {
                        if (o.error) {
                            g()
                        } else {
        ...
        this.isAllowed = function () {
            if (this.fm.selected.length) {
                var d = this.fm.getselected(), c = d.length;
                while (c--) {
                    if (d[c].read) {
                        return true
                    }
        ...
        this.exec = function () {
            ...
            var j = {cmd: "paste", current: this.fm.cwd.hash, src: this.fm.buffer.src, dst: this.fm.buffer.dst, cut: this.fm.buffer.cut};
        ...
        this.exec = function () {
            ...
            if (g.length) {
                this.fm.lockShortcuts(true);
                a('<div><div class="ui-state-error ui-corner-all"><span class="ui-icon ui-icon-alert"/><strong>' + this.fm.i18n("Are you sure you want to remove files?<br /> This cannot be undone!") + "</strong></div></div>").dialog({title: this.fm.i18n("Confirmation required"), dialogClass: "el-finder-dialog", width: 350, close: function () {
                    b.fm.lockShortcuts()
                }, buttons: {Cancel: function () {
                    a(this).dialog("close")
                }, Ok: function () {
                    a(this).dialog("close");
                    var h = {cmd: "rm", current: b.fm.cwd.hash};
                    if (b.fm.jquery > 132) {
                        h.targets = g
                    } else {
                        h["targets[]"] = g
        ...
        this.isAllowed = function (g) {
            if (this.fm.selected.length) {
                var e = this.fm.getselected(), d = e.length;
                while (d--) {
                    if (e[d].rm) {
                        return true
                    }
        ...
        this.exec = function () {
            ...
            f = {name: e, hash: "", mime: "directory", read: true, write: true, date: "", size: 0}, el = this.fm.options.view == "list" ? a(this.fm.view.renderRow(f)).children("td").eq(1).empty().append(input).end().end() : a(this.fm.view.renderIcon(f)).children("label").empty().append(input).end();
            ...
            function d() {
                if (!b.fm.locked) {
                    ...
                    b.fm.ajax({cmd: "mkdir", current: b.fm.cwd.hash, name: g}, function (i) {
                        if (i.error) {
                            el.addClass("ui-selected");
                            return input.select().focus()
                        }
        ...
        this.exec = function () {
            b.fm.unselectAll();
            var i = this.fm.uniqueName("untitled file", ".txt"), e = a('<input type="text"/>').val(i), h = {name: i, hash: "", mime: "text/plain", read: true, write: true, date: "", size: 0}, g = this.fm.options.view == "list" ? a(this.fm.view.renderRow(h)).children("td").eq(1).empty().append(e).end().end() : a(this.fm.view.renderIcon(h)).children("label").empty().append(e).end();
            ...
            function d() {
                if (!b.fm.locked) {
                    ...
                    b.fm.ajax({cmd: "mkfile", current: b.fm.cwd.hash, name: j}, function (l) {
                        if (l.error) {
                            g.addClass("ui-selected");
                            return e.select().focus()
                        }
        ...
        this.exec = function () {
            ...
            k = a(k + "</form>");
            o.append(k.append(l.hide()).prepend(h).append(s)).dialog({dialogClass: "el-finder-dialog", title: b.fm.i18n("Upload files"), modal: true, resizable: false, close: function () {
                b.fm.lockShortcuts()
            }, buttons: {Cancel: function () {
                a(this).dialog("close")
            }, Ok: function () {
                ...
                setTimeout(function () {
                    b.fm.lock();
                    if (a.browser.safari) {
                        a.ajax({url: b.fm.options.url, data: {cmd: "ping"}, error: n, success: n})
                    } else {
                        n()
                    }
                });
            ...
            function n() {
                var v = a('<iframe name="' + g + '" name="' + g + '" src="about:blank"/>'), w = v[0], i = 50, u, e, t;
                v.css({position: "absolute", top: "-1000px", left: "-1000px"}).appendTo("body").bind("load", function () {
                    v.unbind("load");
                    d()
                });
                b.fm.lock(true);
                    ...
                    try {
                        ...
                        if (b.fm.jquery >= 141) {
                            ...
                            if (/^[\],:{}\s]*$/.test(e.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g, "@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, "]").replace(/(?:^|:|,)(?:\s*\[)+/g, ""))) {
                                ...
                                t = {error: "Unable to parse server response"}
                            }
                        }
                    } catch (x) {
                        t = {error: "Unable to parse server response"}
        ...
        this.fm = c;
        this.exec = function () {
            this.fm.ajax({cmd: "duplicate", current: this.fm.cwd.hash, target: this.fm.selected[0]}, function (d) {
                b.fm.reload(d)
            })
        };
        this.isAllowed = function () {
        ...
        this.exec = function () {
            ...
            this.fm.lockShortcuts(true);
            this.fm.ajax({cmd: "read", current: this.fm.cwd.hash, target: d.hash}, function (g) {
                b.fm.lockShortcuts(true);
                var e = a("<textarea/>").val(g.content || "").keydown(function (j) {
                    ...
                    if (j.keyCode == 9) {
                        ...
                        if (a.browser.msie) {
                            ...
                            var i = this.value.substr(0, this.selectionStart), k = this.value.substr(this.selectionEnd);
                            this.value = i + "\t" + k;
                            this.setselectionRange(i.length + 1, i.length + 1)
                        }
                    }
                });
                a("<div/>").append(e).dialog({dialogClass: "el-finder-dialog", title: b.fm.i18n(b.name), modal: true, width: 500, close: function () {
                    b.fm.lockShortcuts()
                }, buttons: {Cancel: function () {
                    a(this).dialog("close")
                }, Ok: function () {
                    var h = e.val();
                    a(this).dialog("close");
                    b.fm.ajax({cmd: "edit", current: b.fm.cwd.hash, target: d.hash, content: h}, function (i) {
                        if (i.target) {
        ...
        this.exec = function (d) {
            var e = {cmd: "archive", current: b.fm.cwd.hash, type: a.inArray(d, this.fm.params.archives) != -1 ? d : this.fm.params.archives[0], name: b.fm.i18n("Archive")};
            if (this.fm.jquery > 132) {
        ...
        this.argc = function () {
            ...
            for (e = 0; e < b.fm.params.archives.length; e++) {
                d.push({"class": "archive", argc: b.fm.params.archives[e], text: b.fm.view.mime2kind(b.fm.params.archives[e])})
            }
            return d
        }
    }, extract: function (c) {
        ...
        this.fm = c;
        this.exec = function () {
            this.fm.ajax({cmd: "extract", current: this.fm.cwd.hash, target: this.fm.getselected(0).hash}, function (d) {
                b.fm.reload(d)
            })
        };
        this.isAllowed = function () {
            var e = this.fm.params.extract, d = e && e.length;
        ...
        this.exec = function () {
            ...
            if (l[0] && l[0].write && l[0].dim) {
                var j = l[0].dim.split("x"), g = parseInt(j[0]), k = parseInt(j[1]), e = g / k;
                ...
                b.fm.lockShortcuts(true);
                var m = a("<div/>").append(a("<div/>").text(b.fm.i18n("Dimensions") + ":")).append(f).dialog({title: b.fm.i18n("Resize image"), dialogClass: "el-finder-dialog", width: 230, modal: true, close: function () {
                    b.fm.lockShortcuts()
                }, buttons: {Cancel: function () {
                    a(this).dialog("close")
                }, Ok: function () {
                    var d = parseInt(iw.val()) || 0, h = parseInt(ih.val()) || 0;
                    if (d > 0 && d != g && h > 0 && h != k) {
                        b.fm.ajax({cmd: "resize", current: b.fm.cwd.hash, target: l[0].hash, width: d, height: h}, function (n) {
                            b.fm.reload(n)
                        })
                    }
                    a(this).dialog("close")
        ...
        this.fm = b;
        this.exec = function () {
            var j, e = this.fm.i18n("helpText"), c, i, g;
            ...
            var k = a("<div/>").html(g).dialog({width: 617, title: this.fm.i18n("Help"), dialogClass: "el-finder-dialog", modal: true, close: function () {
...
(function (a) {
    elFinder.prototype.quickLook = function (l, b) {
        ...
        this.win = a('<div class="el-finder-ql"/>').hide().append(a('<div class="el-finder-ql-drag-handle"/>').append(a('<span class="ui-icon ui-icon-circle-close"/>').click(function () {
            p.hide()
        })).append(this.title)).append(this.ico).append(this.media).append(this.content.append(this.name).append(this.kind).append(this.size).append(this.date).append(this.url).append(this.add)).appendTo("body").draggable({handle: ".el-finder-ql-drag-handle"}).resizable({minWidth: 420, minHeight: 150, resize: function () {
            if (p.media.children().length) {
                var m = p.media.children(":first");
                switch (m[0].nodeName) {
                    case"IMG":
                        var e = m.width(), n = m.height(), i = p.win.width(), s = p.win.css("height") == "auto" ? 350 : p.win.height() - p.content.height() - p.th, q = e > i || n > s ? Math.min(Math.min(i, e) / e, Math.min(s, n) / n) : Math.min(Math.max(i, e) / e, Math.max(s, n) / n);
                        m.css({width: Math.round(m.width() * q), height: Math.round(m.height() * q)});
                        break;
                    case"IFRAME":
                    case"EMBED":
        ...
        this.mimes = {"image/jpeg": "jpg", "image/gif": "gif", "image/png": "png"};
        ...
        this.show = function () {
            if (this.win.is(":hidden") && p.fm.selected.length == 1) {
                g();
                var m = p.fm.selected[0], e = p.fm.view.cwd.find('[key="' + m + '"]'), i = e.offset();
                p.fm.lockShortcuts(true);
                this.win.css({width: e.width() - 20, height: e.height(), left: i.left, top: i.top, opacity: 0}).show().animate({width: 420, height: 150, opacity: 1, top: Math.round(a(window).height() / 5), left: a(window).width() / 2 - 210}, 450, function () {
                    p.win.css({height: "auto"});
                    p.fm.lockShortcuts()
                })
            }
        };
        this.hide = function () {
            if (this.win.is(":visible")) {
                ...
                if (e) {
                    i = e.offset();
                    this.media.hide(200);
                    this.win.animate({width: e.width() - 20, height: e.height(), left: i.left, top: i.top, opacity: 0}, 350, function () {
                        p.fm.lockShortcuts();
                        j();
        ...
        function g() {
            ...
            p.win.css({width: "420px", height: "auto"})
        }
    };
    elFinder.prototype.quickLook.prototype.plugins = {image: new function () {
        ...
        this.show = function (e, d) {
            ...
            function g(k) {
                var j = k.width(), m = k.height(), i = e.win.is(":animated"), l = i ? 420 : e.win.width(), o = i || e.win.css("height") == "auto" ? 315 : e.win.height() - e.content.height() - e.th, n = j > l || m > o ? Math.min(Math.min(l, j) / j, Math.min(o, m) / m) : Math.min(Math.max(l, j) / j, Math.max(o, m) / m);
                e.fm.lockShortcuts(true);
                e.ico.hide();
                k.css({width: e.ico.width(), height: e.ico.height()}).show().animate({width: Math.round(n * j), height: Math.round(n * m)}, 450, function () {
                    e.fm.lockShortcuts()
                })
            }
        }
    ...
    elFinder.prototype.eventsManager = function (d, c) {
        ...
        this.init = function () {
            ...
            this.cwd.bind("click", function (j) {
                ...
                g.fm.quickLook.hide();
                g.fm.ui.showMenu(j)
            }).selectable({filter: "[key]", delay: 300, stop: function () {
                g.fm.updateSelect();
                g.fm.log("mouseup")
            }});
            ...
            if (this.fm.options.places) {
                this.fm.view.plc.click(function (l) {
                    l.preventDefault();
                    var j = a(l.target), k = j.attr("key"), i;
                    if (k) {
                ...
                this.fm.view.plc.droppable({accept: "(div,tr).directory", tolerance: "pointer", over: function () {
        ...
        this.updateNav = function () {
            ...
            a("a:not(.noaccess,.readonly)", this.tree).droppable({tolerance: "pointer", accept: "div[key],tr[key]", over: function () {
                a(this).addClass("el-finder-droppable")
            }, out: function () {
                a(this).removeClass("el-finder-droppable")
            }, drop: function (h, g) {
        ...
        this.updatePlaces = function () {
            this.fm.view.plc.children("li").find("li").draggable({scroll: false, stop: function () {
        ...
        this.updateCwd = function () {
            ...
            }).draggable({delay: 3, addClasses: false, appendTo: ".el-finder-cwd", revert: true, drag: function (h, g) {
                g.helper.toggleClass("el-finder-drag-copy", h.shiftKey || h.ctrlKey)
            }, helper: function () {
                var g = a(this), i = a('<div class="el-finder-drag-helper"/>'), j = 0;
            ...
            }}).filter(".directory").droppable({tolerance: "pointer", accept: "div[key],tr[key]", over: function () {
                a(this).addClass("el-finder-droppable")
            }, out: function () {
                a(this).removeClass("el-finder-droppable")
            }, drop: function (h, g) {
