...
ALTER TABLE `nature_%s` ADD COLUMN `%s` %s NULL;
    ...
    if kwargs['instance']._meta.object_name == 'Entity':
        clear_cache(kwargs['instance'].alias)
    elif kwargs['instance']._meta.object_name == 'Property' and kwargs['created']:
        clear_cache(kwargs['instance'].parent_entity.alias)
...
def get_application_instance(application_alias, request):
    ...
    application_type = ContentType.objects.get(app_label="turbodiesel", model="application")
...
def get_model(request, entity_alias, application_alias):
    ...
    try:
        entity_type = ContentType.objects.get(app_label="turbodiesel", model=entity_alias)
        return entity_type.model_class()
    except ObjectDoesNotExist:
        pass

    # Получение модели шаблона
    try:
    ...
    # Получение модели кастомной сущности
    try:
